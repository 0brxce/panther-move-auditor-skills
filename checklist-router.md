...
| Sui loop over a `Table`/`Bag`/`ObjectTable`/`TableVec`/`dynamic_field` inside a function reachable from an **atomic multi-entity** operation (`settle`, `flush`, `sweep`, `finalize`, `roll`, `close_all`, `migrate`, `value_all`, keeper/cron entry, or any PTB the protocol's own client composes) | force **child-object cache-ceiling review (SUI-46)** — count *distinct* dynamic-field children touched and **sum across every PTB command** (the object-runtime cache is cumulative, not per-command); prove `< 1,000` (`object_runtime_max_num_cached_objects`). Gas headroom is NOT evidence and `sui move test` does NOT enforce this limit — require a node dry-run at projected scale |
...
- **If the chain is Sui AND any shared object exists**, run the SUI-23 stale-package detection ritual (list shared types → confirm `version` field → grep every public-surface function taking `&T`/`&mut T` → confirm version assertion). Treat every historical package version as live attack surface until `CURRENT_VERSION` is bumped and all live shared objects are migrated or otherwise invalidated.
